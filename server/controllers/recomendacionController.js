...
export const getRecomendacion = async (req, res) => {
    try {
        ...
        const recommendations = await fetchRecommendations(interests, price_preference);
        if (recommendations.length === 0) {
            return res.json({ recommendations: [] });
        }

        // Modificar las recomendaciones para que solo devuelvan "activity"
        const modifiedRecommendations = recommendations.map(rec => ({ activity: rec.activity }));
...
const fetchRecommendations = async (interests, price_preference) => {
    ...
    try {
        console.log(`${API_RECOMENDACIONES}/recommend`);
        console.log(interests,price_preference);
